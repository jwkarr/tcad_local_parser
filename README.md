...
Recorder-based pipeline - Identifies verified private note holders from recorder data
TCAD-based pipeline - Generates property owner targets (NOT verified note holders)
Overview
This pipeline processes recorder export CSVs and classifies each row into one of three categories:

note_leads_email_ready.csv - Best candidates, formatted for email campaigns (Instantly/Smartlead)
note_leads_mail_ready.csv - Same leads, formatted for mail campaigns
review_queue.csv - Rows that need manual review due to ambiguity
discarded.csv - Rows filtered out (with reason provided)
Quick Start
...
Example Workflow
Export recorder data from county system to CSV
...
Upload to campaign platform:
Import note_leads_email_ready.csv to Instantly/Smartlead
Launch email campaign
Tune thresholds in filter_notes.py if needed
...
Send note_leads_email_ready.csv to enrichment provider
Provider matches by lead_id and returns emails
Merge emails back using lead_id as the key
Upload enriched CSV to campaign platform
